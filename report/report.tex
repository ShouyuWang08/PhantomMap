...
\begin{abstract}
   ...
   A logistic regression over ten shape and logit features of the
...
\textbf{(ii)} a training-free logistic-regression detector over ten
...
Fig.~\ref{fig:teaser} provides a qualitative overview of the phenomenon. This provides a new perspective on hallucination that focuses on spatial behavior rather than semantic correctness.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{overleaf//figures/fig1_teaser.pdf}
\caption{Comparison of phantom (hallucinated) and honest bounding boxes. Top row shows hallucinated objects absent from the image, while the bottom row shows correctly grounded objects present in the image.}
\label{fig:teaser}
\end{figure}
...
For every ``yes'' event with a valid bbox we extract ten features:
...
All hyperparameters, prompts, parsing rules, and feature definitions are fixed and explicitly described. The use of deterministic decoding (temperature = 0) and a single shared prompt across models ensures that the entire pipeline can be reproduced without access to the original implementation.
...
artefact of one model's grounding supervision. LLaVA contributes only
$71$ phantom boxes; its trend is consistent with Qwen but should be
...
\paragraph{Limitations and future work.} Four caveats apply. First,
...
or redundant. Fourth, the HALP number we cite ($0.787$) comes from
the HALP authors' own evaluation setup; because our data split and
task framing differ, the comparison is a reference point rather than
a controlled head-to-head.

\begin{figure}[!htbp]
\centering
\includegraphics[width=\columnwidth]{report/fig5_failures.pdf}
...
PhantomMap studies hallucination signals in vision--language models. While these signals may help improve model reliability, they could also be misused to make hallucinated outputs harder to detect. Our results also inherit biases from POPE and COCO, including object-frequency and dataset-specific biases, so the findings should not be assumed to generalize to all domains. We release code for academic use and present the method as a transparency tool rather than a way to manipulate model behavior.
